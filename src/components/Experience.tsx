  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-5xl mx-auto flex flex-col items-center">
        <div className="bg-track absolute top-0 w-0.5 bg-cyan-900/30 h-full hidden md:block" />

        {/* Glow Stream (Already has 'energy-glow' class) */}
        <div className="absolute top-0 w-0.5 h-full hidden md:block overflow-hidden">
          <div className="energy-glow w-full bg-linear-to-b from-cyan-400 to-purple-500 shadow-[0_0_20px_#22d3ee] origin-top" />
        </div>

        <div className="w-full flex flex-col gap-24 px-6 md:px-0">
          {experiences.map((exp, i) => (
            <div
              key={i}
              className={`data-panel relative flex items-center w-full ${i % 2 === 0 ? "md:justify-start" : "md:justify-end"}`}
            >
              {/* Connector */}
              <div
                className="hidden md:block absolute w-20 h-px bg-cyan-500/50"
                style={{
              ...
              {/* Neon Card */}
              <div className="w-full md:w-[45%] p-px bg-linear-to-r from-cyan-500/50 to-purple-500/50 rounded-sm">
